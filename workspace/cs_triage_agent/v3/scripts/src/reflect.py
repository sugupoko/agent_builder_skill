...
def _check_apology(body: str, complaint: bool, cfg: dict) -> list:
    """v3: 禁止語（complaint=False 時）と必須語（complaint=True 時）を分離。"""
    apology_cfg = cfg.get("apology", {}) or {}
    forbidden = apology_cfg.get("forbidden_when_no_complaint", ["申し訳", "お詫び", "ご迷惑"]) or []
    if complaint:
        # complaint=True なら apology が含まれているべき（禁止語のいずれかを使う）
        if not any(k in body for k in forbidden):
            return ["missing_apology_for_complaint"]
        return []
    # complaint=False で禁止語が混入したら NG
    return [f"forbidden_apology_when_no_complaint:{k}" for k in forbidden if k in body]


def _check_forbidden_internal(body: str, cfg: dict) -> list:
    """v3 NEW: 内部用語禁止辞書のハードチェック。"""
    forbidden = cfg.get("forbidden_internal_phrases", []) or []
    return [f"forbidden_internal:{p}" for p in forbidden if p in body]
...
def _rule_based_reflect(state: TriageState) -> dict:
    ...
    issues += _check_forbidden_internal(body, cfg)  # v3 NEW
...
{{
  ...
  "reflect_issues": ["<問題があれば短く列挙、なければ空配列>"]
    ...
    try:
        # v2: max_tokens 300 → 800（v1 で 2 件が JSON 切り詰めでパース失敗）
