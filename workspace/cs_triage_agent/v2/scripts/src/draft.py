...
def _build_template_constraints(cfg: dict, complaint: bool, category: str) -> str:
    """v2: complaint=False 時の apology 禁止 / DB 値積極記載 / CAD URL 本文禁止 を明示。"""
    ...
    items = [
        ...
        "- **DB 引き当て結果に在庫数・納期日数・配達予定日・追跡番号がある場合、数値で本文に必ず明記する**（曖昧化禁止）",
    ]
    if complaint:
        items.append(f"- 冒頭の挨拶直後にお詫び: 「{tpl.get('apology_prefix', 'この度はご不便をおかけし、誠に申し訳ございません。')}」")
        items.append("- 内部メモに「スーパーバイザー引き継ぎ要」と明記")
    else:
        # v2 NEW: complaint=False 時は apology 文言禁止
        items.append("- **complaint_smell=False のため、お詫び表現（『申し訳ございません』『ご迷惑をおかけし』『お詫び』）は本文に入れない**。通常応対では「お手数をおかけしますが」程度に留める")
    if category == "cad":
        # v2 NEW: CAD URL 顧客送出禁止
        items.append("- **CAD URL（社内 CDN URL）は顧客向け本文に直接書かない**。本文は「CAD データは担当者より別途ご案内いたします」のテンプレに統一し、URL は internal_memo にのみ記載")
    if category == "tech":
